...
이제 CPU란 도대체 어떤 역할을 어떤 방식으로 수행하는지를 간단히 정리해보도록 하겠습니다. 관련해서는 유튜브에서 [**bRd 3D**](https://www.youtube.com/watch?v=Fg00LN30Ezg) 님의 훌륭한 영상 컨텐츠도 있으니 함께 참고해보시면 좋을 것 같습니다.
...
그렇지만 1945년에 폰 노이만이 **프로그램도 메모리에 저장한다는 개념**을 소개하면서 컴퓨터는 CPU와 메모리만 갖추고, 서로 다른 프로그램은 메모리 공간에서 **프로그램이 저장된 영역만 바꾸면** 되는 아키텍처를 소개하였습니다. [^1] 이처럼 프로그램 자체가 메모리에 저장되는 개념을 [**저장된 프로그램 개념 (Stored-program concept)**](https://en.wikipedia.org/wiki/Stored-program_computer)이라 부르고 이 개념을 사용한 아키텍처를 [**폰 노이만 구조 (Von Neumann Architecture)**](https://en.wikipedia.org/wiki/Von_Neumann_architecture) 라고 부릅니다.

![](von-neumann-architecture.png){: w="500"}
_폰 노이만 구조 도식도_

현존하는 거의 모든 컴퓨터는 이 폰 노이만 구조를 가지고 있다고 할 수 있습니다. PC, 노트북 뿐만 아니라 스마트폰, 태블릿 PC, 다양한 임베디드 기기, 심지어 SSD 같은 PC의 부속품 까지 이 구조로 만들어져 있다고 할 수 있습니다. (SSD도 내부적으로 CPU라 할 수 있는 컨트롤러와 메모리를 가지고 있기 때문입니다)
...
[^1]: <https://en.wikipedia.org/wiki/Von_Neumann_architecture#History>