...
여기까지 해서 ISA와 CPU의 실제 구현체라 할 수 있는 마이크로아키텍처의 개괄적인 내용을 알아보았습니다. 사실 PC CPU를 중점으로 설명하겠다고 호언장담하긴 했지만 CPU 코어 파트에서는 정작 PC CPU가 사용하는 다양한 기법은 모두 생략되었습니다.

안 그래도 분량 조절 안된 글에 해당 내용들까지 넣었다간 정말 감당이 안될 것 같아 정말 기초적인 느낌만이라도 전달하고자 생략한 것이라 할 수 있는데요.<del>제가 잘 모르는 분야라 역량 부족 때문...읍읍</del>. 원래는 2개 파트로 해서 CPU는 마치고자 하였는데, 요즘 CPU가 채택하는 핵심 개념들 몇개가 빠져 있어서 세 번째 편까지 작성하게 될 예정입니다.

다음 편에는 <del>진짜</del> 마지막으로 **파이프라인 (Pipeline)**, **비순차 실행 (Out-of-Order Execution)**, **슈퍼 스칼라 (Superscalar)**을 알아보고 CPU 파트는 마무리 짓도록 하겠습니다. 감사합니다!
